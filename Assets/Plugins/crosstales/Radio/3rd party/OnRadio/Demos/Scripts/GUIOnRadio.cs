...
namespace Crosstales.Radio.OnRadio.Demo
{
   ...
   public class GUIOnRadio : MonoBehaviour
   {
      ...
      [Tooltip("'BaseService' from the scene.")] public Crosstales.Radio.OnRadio.Service.BaseService Service;
      ...
      public void OpenUrl()
      {
         if (Player != null && !string.IsNullOrEmpty(Player.Station?.Station))
      ...
      private void onRecordChange(Crosstales.Radio.Model.RadioStation station, Crosstales.Radio.Model.RecordInfo newrecord)
      {
         ...
         if (Player.Station.GetType() == typeof(Crosstales.Radio.OnRadio.Model.RadioStationExt))
         {
            uidStationIcon = Service.DARStationService((Crosstales.Radio.OnRadio.Model.RadioStationExt)station, true);
