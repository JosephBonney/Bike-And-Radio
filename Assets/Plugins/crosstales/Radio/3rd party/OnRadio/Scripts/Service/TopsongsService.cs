...
namespace Crosstales.Radio.OnRadio.Service
{
   ...
   public class TopsongsService : BaseService
   {
      ...
      private Crosstales.Radio.OnRadio.Model.Genre genre;
      ...
      public Crosstales.Radio.OnRadio.Model.Songs Songs { get; protected set; }
      ...
      protected override IEnumerator query(string id)
      {
         if (!string.IsNullOrEmpty(Token))
         {
            ...
            string apiCall = createUrl(genre == Crosstales.Radio.OnRadio.Model.Genre.All ? Util.Helper.getGenre(Crosstales.Radio.OnRadio.Model.Genre.Music) : Util.Helper.getGenre(genre));
...
#endif
               {
                  Songs = Crosstales.Common.Util.XmlHelper.DeserializeFromString<Crosstales.Radio.OnRadio.Model.Songs>(www.downloadHandler.text, false);

                  if (Songs?.Song.Count > 0)
                  {
                     foreach (Crosstales.Radio.OnRadio.Model.Song song in Songs.Song)
                     {
                        ...
                        Crosstales.Radio.OnRadio.Model.RecordInfoExt record = new Crosstales.Radio.OnRadio.Model.RecordInfoExt(song.Songtitle.Trim(), song.Songartist.Trim(), station);
