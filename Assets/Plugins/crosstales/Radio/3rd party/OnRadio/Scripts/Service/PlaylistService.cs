...
namespace Crosstales.Radio.OnRadio.Service
{
   ...
   public class PlaylistService : BaseService
   {
      ...
      private Crosstales.Radio.OnRadio.Model.Genre genre = Crosstales.Radio.OnRadio.Model.Genre.All;
      ...
      public Crosstales.Radio.OnRadio.Model.Genre Genre
      {
         get => genre;
         set => genre = value;
      }
      ...
      public Crosstales.Radio.OnRadio.Model.Play.Playlist Songs { get; private set; }
      ...
      protected override IEnumerator query(string id)
      {
         if (!string.IsNullOrEmpty(Token))
         {
            ...
            {
               www.timeout = Crosstales.Radio.Util.Constants.MAX_WEB_LOAD_WAIT_TIME;
...
#endif
               {
                  Songs = Crosstales.Common.Util.XmlHelper.DeserializeFromString<Crosstales.Radio.OnRadio.Model.Play.Playlist>(www.downloadHandler.text, false);

                  if (Songs?.Station.Count > 0)
                  {
                     foreach (Crosstales.Radio.OnRadio.Model.Play.Station song in Songs.Station)
                     {
                        Crosstales.Radio.OnRadio.Model.RadioStationExt station = new Crosstales.Radio.OnRadio.Model.RadioStationExt(song.Callsign.Trim(), song.Station_id.Trim());
                        Crosstales.Radio.OnRadio.Model.RecordInfoExt record = new Crosstales.Radio.OnRadio.Model.RecordInfoExt(song.Title.Trim(), song.Artist.Trim(), station);
                  ...
                  {
                     Debug.LogWarning("No songs/stations found.", this);
                  }
               }
               else
               {
      ...
      private string createUrl()
      {
         ...
         if (genre != Crosstales.Radio.OnRadio.Model.Genre.All)
         {
            sb.Append(" @genre ");
            sb.Append(Crosstales.Radio.OnRadio.Util.Helper.getGenre(genre));
         }
