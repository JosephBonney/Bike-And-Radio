   ...
   public class GUIRadioStatic : MonoBehaviour
   {
      ...
      private void Start()
      {
         if (Player != null)
         {
            ...
            Station.text = Crosstales.Common.Util.NetworkHelper.CleanUrl(Player.Station.Station);
...
*/
      ...
      private void onAudioPlayTime(RadioStation station, float _playtime)
      {
         if ((int)_playtime != playtime)
         {
            if (Elapsed != null)
               Elapsed.text = Helper.FormatSecondsToHourMinSec(_playtime);
