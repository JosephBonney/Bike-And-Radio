...
*/
            ...
            {
               if (Config.DEBUG)
                  Debug.Log("+++ RESTART - Point reached: " + Helper.FormatSecondsToHourMinSec(PlayTime), this);
      ...
      private void readStream(ref RadioStation _station, ref bool _playback, ref System.IO.Stream _ms, ref bool _error, ref string _errorMessage, ref RecordInfo _nextRecordInfo, ref float _nextRecordDelay)
      {
         if (_station != null)
         {
            if (_station.Url.CTStartsWith(Constants.PREFIX_HTTP) || _station.Url.CTStartsWith(Constants.PREFIX_HTTPS))
            {
                                    ...
                                    */
                                    ...
                                    if (_nextRecordDelay < Constants.FLOAT_TOLERANCE && recordInfo?.Equals(_nextRecordInfo) == false)
                                    {
                                       if (!string.IsNullOrEmpty(_nextRecordInfo.Info))
                                       {
                                          ...
                                          {
                                             _nextRecordInfo = Context.AllPlayedRecords[Context.AllPlayedRecords.IndexOf(_nextRecordInfo)]; //TODO good idea?
                                          }
                                       }
               ...
               {
                  _error = true;
                  _errorMessage = _station + System.Environment.NewLine + "Could not read url after " + Helper.FormatSecondsToHourMinSec(PlayTime) + "!" + System.Environment.NewLine + ex;
...
*/
      ...
      {
         error = true;
         errorMessage = station + System.Environment.NewLine + "No more data to read after " + Helper.FormatSecondsToHourMinSec(PlayTime) + "! Please restart this station or choose another one.";
         Debug.LogError(errorMessage, this);
      ...
      {
         error = true;
         errorMessage = station + System.Environment.NewLine + "Could not read audio after " + Helper.FormatSecondsToHourMinSec(PlayTime) + "! This is typically a sign of a buffer underun -> Please try to increment the 'ChunkSize' and 'BufferSize':" + System.Environment.NewLine + ex;
         Debug.LogError(errorMessage, this);
