   ...
   public class RadioProviderUser : BaseRadioProvider
   {
      ...
      protected override void init()
      {
         ...
         if (Entry?.EnableSource == true)
         {
            if (!string.IsNullOrEmpty(Entry.FinalPath) && System.IO.File.Exists(Entry.FinalPath))
               StartCoroutine(loadWeb(addCoRoutine(), new RadioEntryURL(Entry, Crosstales.Common.Util.NetworkHelper.ValidURLFromFilePath(Entry.FinalPath), DataFormatURL.Text), true));
      ...
      protected override void initInEditor()
      {
         if (Helper.isEditorMode)
         {
            ...
            {
               if (!string.IsNullOrEmpty(Entry.FinalPath) && System.IO.File.Exists(Entry.FinalPath))
                  loadWebInEditor(new RadioEntryURL(Entry, Crosstales.Common.Util.NetworkHelper.ValidURLFromFilePath(Entry.FinalPath), DataFormatURL.Text), true);
