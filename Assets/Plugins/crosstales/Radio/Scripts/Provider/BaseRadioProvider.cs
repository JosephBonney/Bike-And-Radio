         ...
         if (!string.IsNullOrEmpty(path))
         {
            ...
            try
            {
               ...
               using (System.IO.StreamWriter file = new System.IO.StreamWriter(path))
               {
                  ...
                  file.WriteLine("# © 2015-2022 by " + Constants.ASSET_AUTHOR + " (" + Constants.ASSET_AUTHOR_URL + ")");
                  file.WriteLine("#");
      ...
      protected void fillStationsFromM3U(System.Collections.Generic.List<string> list, BaseRadioEntry entry, int readNumberOfStations = 0, bool suppressDoubleStations = false)
      {
         ...
         for (int ii = 0; ii < list.Count;)
         {
            ...
            if (!line.CTContains(Constants.M3U_EXT_ID)) //EXTM3U?
            {
               ...
               if (!string.IsNullOrEmpty(file) && (!AllowOnlyHTTPS || AllowOnlyHTTPS && isHTTPS(file)))
               {
                  RadioStation station = new RadioStation(entry.ForceName ? entry.Name : string.IsNullOrEmpty(title) ? entry.Name : title.Trim(), file.Trim(), entry.Format == AudioFormat.UNKNOWN ? Helper.AudioFormatFromString(file) : entry.Format, entry.Station, entry.Genres.ToLower(), entry.Bitrate, entry.Rating, entry.Description, entry.Icon, entry.IconUrl, entry.City, entry.Country, entry.Language, entry.ChunkSize, entry.BufferSize, entry.ExcludedCodec);
